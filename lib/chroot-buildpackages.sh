...
chroot_build_packages()
{
	...
	for release in jessie xenial; do
		for arch in armhf arm64; do
			...
			for plugin in $SRC/extras-buildpkgs/*.conf; do
				...
				eval systemd-nspawn -a -q -D $target_dir --tmpfs=/root/build --tmpfs=/tmp:mode=777 --bind-ro $SRC/extras-buildpkgs/:/root/overlay \
...
chroot_installpackages_local()
{
	...
	# -gpg-key="925644A6"
	aptly -keyring="$SRC/extras-buildpkgs/buildpkg-public.gpg" -secret-keyring="$SRC/extras-buildpkgs/buildpkg.gpg" -batch=true -config=$conf \
	...
	local aptly_pid=$!
	cp $SRC/extras-buildpkgs/buildpkg.key $CACHEDIR/$SDCARD/tmp/buildpkg.key
