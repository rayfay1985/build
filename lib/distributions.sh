...
install_common()
{
	...
	if [[ $ROOTFS_TYPE == nfs ]]; then
		...
		if [[ -f $SRC/userpatches/nfs-boot.cmd ]]; then
			...
			cp $SRC/scripts/nfs-boot.cmd.template $CACHEDIR/$SDCARD/boot/boot.cmd
		fi
	fi
	...
	cp $SRC/bin/splash/armbian-u-boot.bmp $CACHEDIR/$SDCARD/boot/boot.bmp
	cp $SRC/bin/splash/armbian-desktop.png $CACHEDIR/$SDCARD/boot/boot-desktop.png

	# execute $LINUXFAMILY-specific tweaks from $BOARD.conf
	[[ $(type -t family_tweaks) == function ]] && family_tweaks

	install -m 755 $SRC/scripts/resize2fs $CACHEDIR/$SDCARD/etc/init.d/
	install -m 755 $SRC/scripts/firstrun  $CACHEDIR/$SDCARD/etc/init.d/
	install -m 644 $SRC/scripts/resize2fs.service $CACHEDIR/$SDCARD/etc/systemd/system/
	install -m 644 $SRC/scripts/firstrun.service $CACHEDIR/$SDCARD/etc/systemd/system/
	...
	cp $SRC/config/71-axp-power-button.rules $CACHEDIR/$SDCARD/etc/udev/rules.d/
